...
const GlossaryCard = ({ term, onFlip }: GlossaryCardProps) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Front */}
        <div className="absolute inset-0 backface-hidden rounded-xl bg-primary border border-border/30 flex items-center justify-center p-4 shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300">
          <h3 className="text-base md:text-lg font-semibold text-primary-foreground text-center leading-tight font-['Sora'] underline decoration-primary-foreground/40 underline-offset-4">
            {term.term}
          </h3>
          <RotateCcw className="absolute bottom-2 right-2 h-3.5 w-3.5 text-primary-foreground/40" />
        </div>
